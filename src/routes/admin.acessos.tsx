...
function AcessosPage() {
  ...
  const aprovar = (r: AccessRequest) => {
    ...
    toast.success(`Acesso concedido a ${r.nome}. Senha temporária enviada por e-mail.`);
  };
  ...
  return (
    <>
      <GovBreadcrumb items={[{ label: "Início", to: "/admin" }, { label: "Solicitações de Acesso" }]} />
      <section className="gov-container pb-12">
        <h1 className="font-display text-2xl mb-1">Solicitações de Acesso</h1>
        <p className="text-sm text-muted-foreground mb-4">
          Pedidos de acesso de pessoas que ainda não constam no AD da AGU. Validação por RH ou Coordenação.
        </p>
            ...
            <article key={r.id} className="gov-card">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">CPF {r.cpf} · Solicitado em {new Date(r.dataSolicitacao).toLocaleDateString("pt-BR")}</div>
                </div>
                <span className="inline-flex rounded-full bg-gov-blue-light px-2.5 py-0.5 text-xs font-semibold text-gov-blue">
                  {r.perfilSolicitado}
                </span>
              </div>
              <div className="mt-3 grid gap-1 text-sm md:grid-cols-2">
                ...
                <div><span className="text-muted-foreground">Cargo pretendido:</span> {r.cargoPretendido}</div>
              ...
              {r.status === "PENDENTE" ? (
                <div className="mt-4 flex justify-end gap-2">
                  <button onClick={() => recusar(r)} className="inline-flex items-center gap-1 rounded-full border border-gov-red px-4 py-2 text-sm font-semibold text-gov-red hover:bg-[oklch(0.97_0.03_27)]"><X className="h-4 w-4" /> Recusar</button>
                  <button onClick={() => aprovar(r)} className="inline-flex items-center gap-1 rounded-full bg-gov-success px-4 py-2 text-sm font-semibold text-white hover:opacity-90"><Check className="h-4 w-4" /> Aprovar acesso</button>
                </div>
              ) : (
                <div className="mt-3 text-xs text-muted-foreground">
                  {r.status === "APROVADO" ? "Aprovado" : "Recusado"} por {r.validadoPor} em {r.dataValidacao ? new Date(r.dataValidacao).toLocaleString("pt-BR") : "—"}
                  {r.motivoRecusa && <> · Motivo: {r.motivoRecusa}</>}
                </div>
              )}
            </article>
          ))}
